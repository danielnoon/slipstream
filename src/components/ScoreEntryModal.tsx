...
export function ScoreEntryModal(props: Props) {
	...
	const participants = useStore(
		(state) => state.rounds.get(id)?.participants!
	);
	...
	const modal = css`
		--min-width: 800px;
		--width: ${modalWidth < window.innerWidth
			? modalWidth
	...
	const hasAnyDuplicates = (): boolean => {
		const matchHasDuplicates = (match: number): boolean => {
			if (round && round.result && round.result.raceResults[match]) {
				const finishes = [
					...round.result.raceResults[match].values(),
		...
		return [...range(racesPerRound)]
			.map((i) => matchHasDuplicates(i))
			.some((e) => e);
	};
	...
	const displayDuplicateErrors = (match: number) => {
		...
		const errorEls = [];
		if (round && round.result && round.result.raceResults[match]) {
			const finishes = [...round.result.raceResults[match].values()].sort(
				(a, b) => a.rank - b.rank
			);
			const groups = groupby(finishes, (finish) => finish.rank);
			for (const [key, value] of groups) {
				const sharedRank = [...value];
				if (sharedRank.length > 1) {
					const guiltyParticipants = [
						...sharedRank.map(
							(result) =>
								participants.find(
									(p) => p.id === result.participant
								)?.name
						),
					];
					guiltyParticipants[guiltyParticipants.length - 1] =
						"and " +
						guiltyParticipants[guiltyParticipants.length - 1];
					const guiltyPartString = guiltyParticipants.join(
						guiltyParticipants.length === 2 ? " " : ", "
					);
					const errorItem = (
						<IonItem className={error} key={key}>
							<IonLabel color="danger" style={{ margin: 4 }}>
								{`Error: ${guiltyPartString} in Race #${
									match + 1
								} cannot ${
									sharedRank.length === 2 ? "both" : "all"
								} finish ${key + 1 + getOrdinal(key + 1)}!`}
							</IonLabel>
						</IonItem>
					);
			...
			<IonContent>
				...
				>
					<IonHeader>
						<IonToolbar>
							<IonTitle>
								Edit Placements for Round {id + 1}
							</IonTitle>
							<IonButtons slot="end">
					...
					{hasAnyDuplicates() && (
						...
						>
							{[...range(courses.length)]
								.map((c) => displayDuplicateErrors(c))
								.flat()}
						</IonList>
					)}
					{
