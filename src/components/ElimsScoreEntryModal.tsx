...
import { generateCourseSelection, uploadRoundResult } from "../algorithms";
import { getOrdinal } from "../utility/rankFormatting";
import { save } from "ionicons/icons";
import { closeCircleOutline } from "ionicons/icons";
import { SeededParticipant } from "../pages/NewEliminations";
import Tournament from "../types/Tournament";
import { getRandomElimsThreshold } from "../data/courseData";
...
export function ElimsScoreEntryModal(props: Props) {
  const { id, isOpen, onClose, participants, tournament } = props;
  const [courses, setCourses] = useState(generateCourseSelection(tournament.platform, getRandomElimsThreshold(), 4))
    ...
    presentSubmitWarning({
      ...
      buttons: [
        "Cancel",
        {
          text: "Yes",
          handler: () => {
            // eliminate participants here
            // increment elims round
            onClose?.();
          },
        },
      ],
    ...
    <IonModal isOpen={isOpen} className={modal} onDidDismiss={onClose}>
      <IonContent>
        ...
        >
          <IonHeader>
            <IonToolbar>
              <IonTitle>Edit Placements for Elimination Round {id + 1}</IonTitle>
              <IonButtons slot="end">
          ...
          <IonGrid className={grid(participants.length)}>
            <div></div>
            {participants.map((seededPart) => (
              <IonItem key={seededPart.id}>
                <IonListHeader>
                  <IonLabel>{seededPart.participant.name}</IonLabel>
                  
                    <IonButton onClick={() => {
                      getState().deleteParticipant(seededPart.id);
                    }}>
