...
export function Eliminations() {
	...
	useEffect(() => {
		requestAnimationFrame(() => {
			const els = seededParticipants
				.map((p) => document.getElementById(`p-${p.id}`))
				.filter((el) => el);
			const height = els[0]?.getBoundingClientRect().height;
			if (height) {
				const activeHeight =
					active.length * (height + PADDING) + PADDING;
	...
	useIonViewDidEnter(() => {
		...
		const seededParticipants: SeededParticipant[] = participants
			.slice()
	...
	function eliminatePlayer(id: number) {
		const newBelow = [
			seededParticipants!.find((p) => p.id === id)!,
			...below,
		];
								...
								{active.find((a) => participant.id === a.id) &&
									active.length > 3 && (
										<IonButton
											...
											className={eliminateButton}
											onClick={() =>
												eliminatePlayer(participant.id)
											}
										>
